...
<?php
	...
	$this->lists['order'] = $this->escape($this->state->get('filter.ordering', 'ID_EVAL_ACTION_ACTIV'));// default order is primary key
...
?>
<form action="<?php echo JRoute::_('index.php?option=com_syspat'); ?>" method="post" name="adminForm" id="adminForm">
		...
		<div class="clearfix">
			...
			<div class="js-stools-container-bar">
				...
				<div class="btn-wrapper input-append">
					...
					<button class="btn hasTooltip" type="submit" data-original-title="<?php echo JText::_('JSEARCH_FILTER_LABEL'); ?>"><i class="icon-search"></i></button>
				</div>
...
<?php endif; ?>
	...
	<th class="jcb_fieldDiv jcb_fieldLabel"><?php $numCols++; ?>
		<?php echo JHTML::_('grid.sort', 'Date évaluation', 'EVAL_ACT_DATE', $this->lists['order_Dir'], $this->lists['order']); ?>
	</th>

	<th class="jcb_fieldDiv jcb_fieldLabel"><?php $numCols++; ?>
		<?php echo JHTML::_('grid.sort', 'Réf. Act', 'ACTION_ACTIV_REF', $this->lists['order_Dir'], $this->lists['order']); ?>
	</th>
	<th class="jcb_fieldDiv jcb_fieldLabel"><?php $numCols++; ?>
		<?php echo JHTML::_('grid.sort', 'Code domaine', 'DOMAIN_CODE', $this->lists['order_Dir'], $this->lists['order']); ?>
	</th>
	<th class="jcb_fieldDiv jcb_fieldLabel"><?php $numCols++; ?>
		<?php echo JHTML::_('grid.sort', 'Code sous domaine', 'DIMENSION_CODE', $this->lists['order_Dir'], $this->lists['order']); ?>
	</th>
		...
		Code indicateur PEFA
			...
			<th width="20" class="jcb_fieldDiv jcb_fieldLabel nowrap"><?php $numCols++; ?>
				...
				<?php if ($this->auth('core.edit') && ($this->lists['order']=='ordering')): ?>
					<?php echo JHtml::_('grid.order',  $this->rows, 'filesave.png', 'evaluation_list.saveorder'); ?>
				<?php endif;?>				
			</th>
			<?php endif; ?>
	...
	for ($i=0, $n=count( $this->rows ); $i < $n; $i++)	{
		$row = &$this->rows[$i];
		$checked = JHTML::_('grid.id', $i, $row->ID_EVAL_ACTION_ACTIV);
		$link = JRoute::_( 'index.php?option=com_syspat&task=action_activ.edit&id='. $row->ID_EVAL_ACTION_ACTIV);
		$k = 1 - $k;
		?>
		<tr class="<?php echo "row$k"; ?>">
			...
			<td>
				<?php if (isset($row->checked_out) && ($row->checked_out)) : ?>
					<?php echo JHtml::_('jgrid.checkedout', $i, 'someone...', $row->checked_out_time, 'evaluation_list.', $this->auth('author.checkin')); ?>
				<?php endif; ?>				
	...
	<td class="jcb_fieldDiv jcb_fieldValue">
		<?php echo $row->EVAL_ACT_DATE; ?>
	</td>
	<td class="jcb_fieldDiv jcb_fieldValue">
		<?php echo $row->ACTION_ACTIV_REF; ?>
	</td>
	<td class="jcb_fieldDiv jcb_fieldValue">
		<?php echo $row->DOMAIN_CODE; ?>
	</td>
	<td class="jcb_fieldDiv jcb_fieldValue">
		<?php echo $row->DIMENSION_CODE; ?>
	</td>
	<td class="jcb_fieldDiv jcb_fieldValue">
		<?php echo $row->DESCRIPTION; ?>
	</td>
	<td class="jcb_fieldDiv jcb_fieldValue">
		<?php echo $row->INDICATOR_PEFA_CODE; ?>
	</td>
			...
			<td class="order nowrap">
				...
				?>
					<span><?php echo $page->orderUpIcon( $i, $i>0, 'evaluation_list.'.$up, JText::_( 'JLIB_HTML_MOVE_UP' ), $enable ); ?></span>
					<span><?php echo $page->orderDownIcon( $i, $n, $i<$n, 'evaluation_list.'.$down, JText::_( 'JLIB_HTML_MOVE_DOWN' ), $enable ); ?></span>					
			...
			<td>
				<?php if($this->j3x): ?>
					<?php echo JHtml::_('jgrid.published', $row->published, $i, 'evaluation_list.', true); ?>
				<?php else: ?>
					<?php echo JHTML::_('grid.published', $row, $i, 'tick.png', 'publish_x.png', 'evaluation_list.'); ?>
				<?php endif; ?>
			</td>
			<?php endif; ?>
		</tr>
...
<input type="hidden" name="option" value="com_syspat" />
<input type="hidden" name="task" value="" />
<input type="hidden" name="boxchecked" value="0" />
<input type="hidden" name="view" value="evaluation_list" />
