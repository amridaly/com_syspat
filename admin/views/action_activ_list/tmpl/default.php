...
<?php
	...
	$this->lists['order'] = $this->escape($this->state->get('filter.ordering', 'ID_ACTION_ACTIV'));// default order is primary key
...
<?php endif; ?>
		...
		Périodicité
			...
			<th width="20" class="jcb_fieldDiv jcb_fieldLabel nowrap"><?php $numCols++; ?>
				...
				<?php if ($this->auth('core.edit') && ($this->lists['order']=='ordering')): ?>
					<?php echo JHtml::_('grid.order',  $this->rows, 'filesave.png', 'action_activ_list.saveorder'); ?>
				<?php endif;?>				
			</th>
			<?php endif; ?>
	...
	for ($i=0, $n=count( $this->rows ); $i < $n; $i++)	{
		$row = &$this->rows[$i];
		$checked = JHTML::_('grid.id', $i, $row->ID_ACTION_ACTIV);
		$link = JRoute::_( 'index.php?option=com_syspat&task=action_activ.edit&id='. $row->ID_ACTION_ACTIV);
		$k = 1 - $k;
		?>
		<tr class="<?php echo "row$k"; ?>">
			...
			<td>
				<?php if (isset($row->checked_out) && ($row->checked_out)) : ?>
					<?php echo JHtml::_('jgrid.checkedout', $i, 'someone...', $row->checked_out_time, 'action_activ_list.', $this->auth('author.checkin')); ?>
				<?php endif; ?>				
	...
	<td class="jcb_fieldDiv jcb_fieldValue">
		<?php echo $row->PERTINENCE; ?>
	</td>
	<td class="jcb_fieldDiv jcb_fieldValue">
		<?php echo $row->INDICATOR_PEFA_CODE; ?>
	</td>
    <td class="jcb_fieldDiv jcb_fieldValue">
		<?php echo $row->IS_RENEWAL; ?>
	</td>	
	<td class="jcb_fieldDiv jcb_fieldValue">
		<?php echo $row->FREQUENCY; ?>
			...
			<td class="order nowrap">
				...
				?>
					<span><?php echo $page->orderUpIcon( $i, $i>0, 'action_activ_list.'.$up, JText::_( 'JLIB_HTML_MOVE_UP' ), $enable ); ?></span>
					<span><?php echo $page->orderDownIcon( $i, $n, $i<$n, 'action_activ_list.'.$down, JText::_( 'JLIB_HTML_MOVE_DOWN' ), $enable ); ?></span>					
			...
			<td>
				<?php if($this->j3x): ?>
					<?php echo JHtml::_('jgrid.published', $row->published, $i, 'action_activ_list.', true); ?>
				<?php else: ?>
					<?php echo JHTML::_('grid.published', $row, $i, 'tick.png', 'publish_x.png', 'action_activ_list.'); ?>
				<?php endif; ?>
			</td>
			<?php endif; ?>
		</tr>
...
<input type="hidden" name="view" value="action_activ_list" />
